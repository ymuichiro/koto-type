...
enum LaunchAtLoginManager {
    ...
    static func setEnabled(_ enabled: Bool) -> Bool {
        ...
        guard Self.canManageLaunchAtLogin(bundlePath: bundlePath) else {
            Self.cleanupUnsupportedRuntimeRegistration(service: service, bundlePath: bundlePath)
            Logger.shared.log(
        ...
        do {
            ...
            Logger.shared.log("LaunchAtLoginManager: set enabled=\(enabled), status=\(Self.statusDescription(service.status))")
    ...
    @available(macOS 13.0, *)
    private static func cleanupUnsupportedRuntimeRegistration(service: SMAppService, bundlePath: String) {

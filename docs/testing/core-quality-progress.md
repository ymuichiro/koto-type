...
ベンチマークCLIから純音の短尺・長尺既定入力を撤去。`--short-audio`未指定ならモデル起動前に終了コード2で拒否し、長尺入力省略時の反復WAVは一時領域にだけ生成する。回帰2件、Python `make test-all`、Ruff、ty、実CLI拒否を確認済み。この修正は評価入力の妥当性を守るもので、ASR認識品質を改善した証拠ではない。
...
- 固定入力は[Common Voice Japanese](https://huggingface.co/datasets/FluidInference/cv-corpus-25.0-ja) 30件（公開manifest revision `365b7654cd582e20e8000921ef7b0e32caa1906a`、CC0-1.0、匿名speaker group 23組・1組あたり最大3件）と[LibriSpeech dev-clean](https://us.openslr.org/12/)英語30件（CC BY 4.0、3話者・各10件）。手動再確認後のタグ件数は疑問7、明示的否定5、数値5、英語固有名詞6、15文字以下11。以前のタグは `cv-02/13/16` を否定、`cv-18` を数値と誤分類し、`cv-21/22` の疑問文を欠落させていた。`cv-11` は明示的否定でなく可能性表現、`cv-09` は文中疑問で文末疑問ではない。音声と参照文を含む選定manifestはローカル `/tmp/kototype-goals-XdWea0/{cv-balanced-v1/manifest.json,english-manifest.json}` にあり、リポジトリには未収録。このためPRだけでは同じ60件を再構成できず、再現可能なfixture化は未達。
...
- Turbo→Large-v3の順で、日本語weighted CERは `0.6028 → 0.2605`、英語は `0.0128 → 0.0125`。この実験の日本語タグ別CERは、manifestの誤分類が判明し、発話単位の出力も保存していないため撤回する。訂正タグでのモデル別集計は再計算不能であり、上記全体CER・失敗率から意味保持を推定しない。空/失敗応答は `36/180 → 6/180`（Turbo: unreliable 30 + insufficient_audio 6、Large-v3: insufficient_audio 6）。疑問符保持は `9/18 → 12/18`。自動言語の不一致は日本語で `9 → 6`、未判定は両方 `6`（いずれも3反復を含む）。
